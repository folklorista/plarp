...
@model({
  settings: {idInjection: false, postgresql: {schema: 'organizing', table: 'tail_x_task'}}
})
export class TailXTask extends Entity {
  @property({
    type: 'number',
    required: true,
    scale: 0,
  ...
  @property({
    type: 'number',
    scale: 0,
    postgresql: {columnName: 'id_tail', dataType: 'integer', dataLength: null, dataPrecision: null, dataScale: 0, nullable: 'YES'},
  })
  idTail?: number;
  ...
  constructor(data?: Partial<TailXTask>) {
    super(data);
  }
}

export interface TailXTaskRelations {
  // describe navigational properties here
}

export type TailXTaskWithRelations = TailXTask & TailXTaskRelations;
